  ...
  return (
    <div>
      <div className="mb-5 sm:mb-7">
        <h1 className="text-lg sm:text-xl font-semibold text-gray-900">Tổng quan</h1>
        <p className="text-xs sm:text-sm text-gray-400 mt-0.5">Dữ liệu cập nhật theo thời gian thực</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-5 sm:mb-6">
        <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-5">
          <p className="text-sm font-medium text-gray-900 mb-3 sm:mb-4">Phân bổ doanh thu</p>
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-gray-200 p-4 sm:p-5">
          <p className="text-sm font-medium text-gray-900 mb-3 sm:mb-4">Top truyện được nghe nhiều</p>
          <div className="space-y-1">
            {top.map((s, i) => (
              <div key={s.id} className="flex items-center gap-2.5 sm:gap-3 py-2 border-b border-gray-50 last:border-0">
                <span className="text-xs text-gray-400 w-4 text-right">{i + 1}</span>
                <div className="flex-1 min-w-0">
                  <p className="text-xs sm:text-sm text-gray-800 truncate">{s.title}</p>
                  <p className="text-[10px] sm:text-xs text-gray-400">{GENRE_LABEL[s.genre] ?? s.genre}</p>
                </div>
                <div className="text-right flex-shrink-0">
                  <p className="text-xs sm:text-sm font-medium text-gray-800">{s.total_plays.toLocaleString()}</p>
                  <p className="text-[10px] sm:text-xs text-gray-400">{s.unique_listeners.toLocaleString()} người</p>
                </div>
              </div>
            ))}
          ...
          <table className="w-full text-xs sm:text-sm">
            <thead><tr className="border-b border-gray-50">
              {["Người dùng", "Loại", "Số tiền", "Trạng thái", "Thời gian"].map(h => (
                <th key={h} className="pb-2 text-left text-[10px] sm:text-xs font-medium text-gray-400 pr-3 sm:pr-4">{h}</th>
              ))}
            </tr></thead>
            <tbody>
              {txns.map(t => (
                <tr key={t.id} className="border-b border-gray-50 hover:bg-gray-50 last:border-0">
                  <td className="py-2 sm:py-2.5 pr-3 sm:pr-4"><p className="text-gray-800 truncate max-w-[120px] sm:max-w-[150px]">{t.name ?? t.email}</p><p className="text-[10px] sm:text-xs text-gray-400 truncate max-w-[120px] sm:max-w-[150px]">{t.email}</p></td>
                  <td className="py-2 sm:py-2.5 pr-3 sm:pr-4 text-gray-600 whitespace-nowrap">{TX_TYPE_LABEL[t.type] ?? t.type}</td>
                  <td className="py-2 sm:py-2.5 pr-3 sm:pr-4 font-medium text-gray-800 whitespace-nowrap">{fmtVnd(t.amount_vnd)}</td>
                  <td className="py-2 sm:py-2.5 pr-3 sm:pr-4"><Badge variant={STATUS[t.status] ?? "neutral"}>{t.status === "paid" ? "Đã TT" : t.status === "pending" ? "Chờ TT" : t.status === "failed" ? "Thất bại" : "Đã huỷ"}</Badge></td>
                  <td className="py-2 sm:py-2.5 text-gray-400 text-[10px] sm:text-xs whitespace-nowrap">{fmtDateTime(t.created_at)}</td>
                </tr>
              ))}
              {txns.length === 0 && <tr><td colSpan={5} className="py-8 sm:py-10 text-center text-sm text-gray-400">Chưa có giao dịch</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
